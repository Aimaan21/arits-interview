...
const dataSourceOptions: DataSourceOptions = {
  ...
  //synchronize: process.env.NODE_DB_ENV ? ( process.env.NODE_DB_ENV == 'development' ? true : false) : false,
  synchronize: false,
...
const centralDbBootstrap = async () => {
  return db.initialize().then(() => {console.log('central_database connected'); return}).catch(err => {throw err});
}

export { db, dataSourceOptions, SnakeNamingStrategy, centralDbBootstrap };